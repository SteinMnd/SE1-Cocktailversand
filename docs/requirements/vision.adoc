= Vision: {project-name}
Sten Mende sten.mende@stud.htw-dresden.de
{localdatetime}
...
Dieses Visions-Dokument bezieht sich auf das System {project-system-name}, das von Team S entwickelt wird. Das System wird es dem Barbetreiber erlauben, Online-Versandt zu nutzen, um damit eine größere Kundenzahl zu erreichen.
...
Das Projekt zielt darauf aus, einen Online-Shop zu eröffnen, um 
...
[cols="1,3"]
|===
| Das Problem | Keine Webpräsenz/ Datenbank
| betrifft | den Barbesitzer
| die Auswirkung davon ist | kleine Kundschaft
| eine erfolgreiche Lösung wäre | Online-Markt, wo Kunden sich Cocktails liefern lassen können
|===
